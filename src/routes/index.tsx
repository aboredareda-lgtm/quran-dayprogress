...
function Index() {
  ...
  return (
    <main className="pattern-cream screen-fill mx-auto overflow-x-hidden sm:max-w-[26rem]">
      ...
      <section className="relative z-10 -mt-8 px-3 pb-2 min-[380px]:px-4 tall:-mt-10">
        <div className="shadow-soft pattern-cream rounded-[1.75rem] border-2 border-gold/45 p-2.5 text-center text-card-foreground tall:rounded-[2rem] tall:p-4">
          ...
          {!loaded ? (
            ...
            <>
              <p className="font-display text-[1.5rem] font-bold leading-tight text-primary tall:mt-2 tall:text-[1.9rem]">
                سورة {getSurah(last.surah).name}
              </p>
              <p className="mt-0.5 text-base font-bold tall:mt-1 tall:text-xl">
                الآية {last.ayah}
                <span className="mr-2 text-xs font-medium text-muted-foreground tall:text-base">
                  من {getSurah(last.surah).ayahs}
                </span>
              </p>
              <p className="mt-1.5 inline-block rounded-full border-2 border-gold/60 bg-secondary px-3.5 py-0.5 text-lg font-bold text-secondary-foreground tall:mt-2 tall:px-5 tall:py-1 tall:text-2xl">
          ...
          <button
            onClick={openEditor}
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-full border-2 border-gold/60 bg-primary px-4 py-2 text-sm font-bold text-primary-foreground transition-transform active:scale-[0.98] tall:mt-3 tall:gap-3 tall:py-3 tall:text-base"
          >
        ...
        <div className="pattern-cream mt-1.5 grid grid-cols-2 divide-x divide-gold/30 rounded-2xl border border-gold/40 text-center tall:mt-2 tall:rounded-3xl">
          {[
            { value: daysTracked, label: "أيام المتابعة" },
            { value: entries.length, label: "مرات التسجيل" },
          ].map((s) => (
            <div key={s.label} className="px-2 py-1.5 tall:py-2.5">
        ...
        <Link
          to="/history"
          className="pattern-cream mt-1.5 flex items-center justify-between rounded-2xl border border-gold/40 px-4 py-2 text-sm font-bold text-primary tall:mt-2 tall:rounded-3xl tall:px-5 tall:py-4 tall:text-base"
        >
        ...
        <p className="mt-1.5 flex items-center justify-center gap-2 text-center text-[0.65rem] leading-4 text-muted-foreground tall:mt-3 tall:text-xs">
